...
#include <kernel/rsrc.h>
	...
	#define ISSUE_LWO		USR_SYSTEM+"objectd/lib/issue_lwo"
	#define LOGD			"/kernel/sys/driver"
	/* I don't want any debug-statements -- rm "TURNED_OFF_" to enable */
	#define write_syslog(x,y)	TURNED_OFF_message(x + "\n")
	/* TODO: Add functions for what is called from here (suspension) */
	#define TELNETD			"/usr/System/httpd/binaryd"
#endif /* __PHANTASMAL__ */
  ...
  if(previous_program() == DRIVER) { 
    ...
    if(dest_issues[objname] || dest_issues[index]) {
	  ...
	  LOGD->write_syslog("Name '" + STRINGD->mixed_sprint(objname)
			   ...
			   LOG_VERBOSE);
#else
	  LOGD->write_syslog("Name '" + objname
			   + "' already in dest_issues.",
			   LOG_VERBOSE);
#endif
...
void do_initial_obj_setup(void) {
  ...
  count_clones(owners);
  LOGD->write_syslog("objectd: do_initial_obj_setup done", 0);
}
...
string report_on_object(string spec) {
  ...
  LOGD->write_syslog("report_on_object()\n", 0);
