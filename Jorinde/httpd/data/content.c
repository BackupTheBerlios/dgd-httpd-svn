 ...
 * Version:     $Id: content.c 226 2004-06-20 21:58:43Z romland $
...
int initialize(string str)
{
	...
	if(count < 2) {
		SYSLOG("WARNING ?: Setting all to content\n");
		headers = "";
		content = str;
	} else {
		headers = headers;
...
#endif
	SYSLOG("Leaving content->initialize()\n");
	return started;
}
...
int get_complete()
{
	...
	if(expected_length == get_content_length() || (is_multipart() && ends_with(
		...
		SYSLOG("All content (" + get_content_length() + ") received.\n");
		return TRUE;
	}
	return FALSE;
}
