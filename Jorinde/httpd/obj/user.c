 ...
 * Version:	 $Id: user.c 255 2004-07-09 14:45:41Z romland $
...
static object create_response(object request)
{
	...
	response->set_header("Expires", 
							datetime(time() + (60*60), app->get_timezone())
						);
...
nomask int send_headers(object response)
{
	...
	if(response->get_content_length() == 0) {
		...
		case 305..999 :
			...
			/*
			 * TODO: If whatever called this function has already decided that
			 * they don't want to send any content, then user will never see
			 * any error page. How shall we fix this?
			 */
		}
	}
...
SYSLOG("logged in name is: " + query_name() + "\n");
	...
	/* POST requests don't need to set content-length */
