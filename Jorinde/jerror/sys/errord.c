 ...
 * Version:     $Id: errord.c 236 2004-06-21 22:19:14Z romland $
...
void compile_error(string file, int line, string error)
{
	...
	for(i = 0; i < sizeof(tmphooks); i++) {
		...
		if(hook[ERR_DAEMON] == nil) {
			driver->message(text::compile_error_tostring(file, line, error));
			continue;
		}
...
void runtime_error(string error, int caught, mixed **trace)
{
	...
	for(i = 0; i < sizeof(tmphooks); i++) {
		...
		if(hook[ERR_DAEMON] == nil) {
			driver->message(runtime_error_tostring(error, caught, trace));
			continue;
		}
